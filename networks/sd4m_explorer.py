...
class Skill_Explorer:
    def __init__(self, state_len, action_len, goal_len, name="",
        ...
        with tf.variable_scope(self.name): 
            ...
            with tf.variable_scope("Explorer"):
                stategoal = tf.concat([self.input_state, self.input_goal], axis=1)
                next_stategoal = tf.concat([self.input_next_state, self.input_goal], axis=1)

                self.log_alpha = tf.Variable(0., trainable=True)
                self.alpha = tf.exp(self.log_alpha)
                self.explorer_policy = GaussianPolicy("explorer_policy", state_len + goal_len, action_len, policy_hidden_len, hidden_nonlinearity=policy_hidden_nonlinearity,
                    input_tensor=stategoal, output_tanh=True)
                self.explorer_next_policy = GaussianPolicy("explorer_policy", state_len + goal_len, action_len, policy_hidden_len, hidden_nonlinearity=policy_hidden_nonlinearity,
                    input_tensor=next_stategoal, output_tanh=True, reuse=True)

                self.explorer_qvalue1 = MLP("explorer_qvalue1", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.input_action)
                self.explorer_qvalue2 = MLP("explorer_qvalue2", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.input_action)
                self.explorer_qvalue1_policy = MLP("explorer_qvalue1", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.explorer_policy.reparameterized, reuse=True)
                self.explorer_qvalue2_policy = MLP("explorer_qvalue2", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.explorer_policy.reparameterized, reuse=True)

                self.explorer_qvalue1_target = MLP("explorer_qvalue1_target", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=next_stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.explorer_next_policy.reparameterized)
                self.explorer_qvalue2_target = MLP("explorer_qvalue2_target", state_len + goal_len, 1, value_hidden_len, hidden_nonlinearity=value_hidden_nonlinearity,
                    input_tensor=next_stategoal, additional_input=True, additional_input_dim=action_len, additional_input_tensor=self.explorer_next_policy.reparameterized)
                ...
                Q_target = tf.stop_gradient(self.input_reward + (min_next_Q - self.explorer_next_policy.log_pi * self.alpha) * policy_gamma)

                self.explorer_qvalue1_optimizer = tf.train.AdamOptimizer(value_lr)
                self.explorer_qvalue1_loss = tf.reduce_mean((self.explorer_qvalue1.layer_output - Q_target) ** 2)
                ...
                self.explorer_qvalue2_loss = tf.reduce_mean((self.explorer_qvalue2.layer_output - Q_target) ** 2)
                ...
                self.explorer_policy_loss = tf.reduce_mean(self.explorer_policy.log_pi * self.alpha - mean_Q)  + self.explorer_policy.regularization_loss * policy_reg
    ...
    def get_action(self, input_state, input_goal, discrete=False):
        input_list = {self.input_state : [input_state], self.input_goal : [input_goal]}
    ...
    def optimize_batch(self, input_state, input_next_state, input_action, input_goal, input_reward, input_iter=None):
