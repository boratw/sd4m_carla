    ...
    with sess.as_default():
        ...
        for task in range(7):
            ...
            with open(log_name + "task_" + str(task) + "_score.txt", 'w') as f:
                za_list = [i * 2.5 - 10. for i in range(12)]
                for za in za_list:
                    env_num = 100
                    action_latent = []
                    vehicles_list = []
                    for x, y in itertools.product(range(10), range(10)):                                                                                                                                                                                                                           
                        ...
                        action_latent.append([0.1, 1.9, za + (x // 2) * 0.1 + (y // 2) * 0.5])
                    ...
                    for i, actor in enumerate(vehicles_list):
                        tr = actor.get_transform()
                        first_pos.append([tr.location.x, tr.location.y])
                        first_yaw.append([np.sin((tr.rotation.yaw + yaw_diff) * -0.017453293), np.cos((tr.rotation.yaw + yaw_diff) * -0.017453293)])
                    ...
                    for x, y in itertools.product(range(0, 10, 2), range(0, 10, 2)):
                        ...
                        for j in [i, i + 1, i + 10, i + 11]:
                            ...
                            px, py = rotate(tr.location.x - first_pos[j][0], tr.location.y - first_pos[j][1], first_yaw[j][0], first_yaw[j][1])
                            tx += px
                            ty += py
                            tx2 += px ** 2
                            ty2 += py ** 2
