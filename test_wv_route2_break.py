...
def get_control(actor, action, steer=0):
                    ...
                    for controller in [0, 1]:
                        ...
                        for step in range(3000):
                            if controller < 1:
                                ...
                                actions = explorer[0].get_action(states, goals, True)
                            vehiclecontrols = []
                            for i, actor in enumerate(vehicles_list):
                                if route_target_iter[i] < break_time :
                                    ratio_fl = 100.
                                    ratio_fr = 100.
                                    ratio_rl = 100.
                                    ratio_rr = 100.
                        ...
                        with open(log_name + "task_" + str(task) + "break_" + str(break_time) + "_control_" + str(controller) + "_route_" + str(route_it) + ".json", 'w') as f:
                            json.dump({"pos" : log_pos, "vel" : log_vel, "distance" : log_distance, "routeit" : log_routeit,
                                        "wheel" : log_wheel, "action" : log_action, "zt" : log_task}, f, indent=2)
