...
log_name = "train_log/Train3_6_2/log_" + datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
...
try:
    with sess.as_default():
        learner = Skill_Learner(state_len, action_len, goal_len, traj_len, traj_track_len, latent_len, latent_body_len, task_num, learner_unity=0.02, learner_diverse=0.01,
            learner_lr=0.0001, sampler_lr=0.0001, sampler_disperse=0.1)
        explorer = [Skill_Explorer(state_len,  action_len, goal_len, name=str(i), policy_gamma=0.95) for i in range(task_num)]
            ...
            for task in range(task_num):
                ...
                for step in range(1000):
                    ...
                    if step % traj_len == 0:
                        ...
                        traj_broken = [False for _ in range(env_num)]
                    ...
                    for i, actor in enumerate(vehicles_list):
                        ...
                        if broken_vector[i] == 0:
                            if random.random() < 0.005:
                                ...
                                broken_value[i] = random.random() * 0.6 - 0.2
                                traj_broken[i] = True
                        else:  
                            broken_vector[i] -= 1
                    ...
                    for i, actor in enumerate(vehicles_list):
                        ...
                        prevreward = np.sqrt((current_goal[i][0] - states[i][0]) ** 2 + (current_goal[i][1] - states[i][1]) ** 2)
                        reward = (prevreward - np.sqrt((current_goal[i][0] - px) ** 2 + (current_goal[i][1] - py) ** 2)) * 4.
                        vel_panelty = np.sqrt(v.x ** 2 + v.y ** 2) * 0.5 - 5.
                        if vel_panelty < 0.:
                            vel_panelty = 0.
                        act_panelty = ((actions[i][0] - actions[i][2]) ** 2 + (actions[i][1] - actions[i][3]) ** 2) * 0.1 


                        cur_reward[task] += reward
                        cur_reward_num[task] += 1.
                        survive = (abs(tr.rotation.roll) < 20) and (abs(tr.rotation.pitch) < 20)
                        if survive_vector[i] :
                            if survive:
                                history_policy[task].append([states[i], nextstates[i][:], actions[i], current_goal[i], [0.1 + reward - vel_panelty - act_panelty], [1.]])
                            else:
                                history_policy[task].append([states[i], nextstates[i][:], actions[i], current_goal[i], [-10.], [0.]])
                    ...
                    if step % traj_len == traj_len - 1:
                        for i, actor in enumerate(vehicles_list):
                            ...
                            if skill_moved > 5. and survive_vector[i] and not traj_broken[i]:
